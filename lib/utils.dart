...
class Utils{
 static String clearMask(String codigo) {
  return codigo.replaceAll(RegExp(r'/( |.|-)'), '');
 ...
 /*
  ...
  três primeiros amarrados por DVs e calculados pelo módulo 10, conforme segue:
    ...
    (zero)
*/
  static int modulo10(String bloco)
  {
    var codigo = _convertBlockToList(bloco);
    int index = 0;
   final somatorio = codigo.fold<int>(0, (acc, current) {
    int soma = current * (((index + 1) % 2) + 1);
    soma = (soma > 9 ? (soma / 10).truncate() + (soma % 10) : soma);
    index++;
    return acc + soma;
  });
   return ((somatorio / 10).ceil() * 10) - somatorio;
  }
/*
...
*/
  static int modulo11Bancario(String bloco)
  {
   int multiplicador = 2;
   var codigo = _convertBlockToList(bloco);
   final somatorio = codigo.fold<int>(0, (acc, current) {
    final soma = current * multiplicador;
    multiplicador = multiplicador == 9 ? 2 : multiplicador + 1;
    return soma + acc;
  });
  final restoDivisao = somatorio % 11;
  final dv = 11 - restoDivisao;
  if (dv == 0 || dv == 10 || dv == 11)
  {
  return 1;
  }
...
*/
 ...
 static int modulo11Arrecadacao(String bloco) {
  int multiplicador = 2;
  var codigo = _convertBlockToList(bloco);
  final somatorio = codigo.fold<int>(0, (acc, current) {
    final soma = current * multiplicador;
 ...
 static Iterable<int> _convertBlockToList (String bloco)
 {
  var codigoList = bloco.split('').reversed;
  return codigoList.map((e) => int.parse(e));
 }
}